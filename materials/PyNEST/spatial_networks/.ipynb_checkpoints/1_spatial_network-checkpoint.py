...
conn_inh = {'rule': 'fixed_indegree', 'indegree': 25}
# -

# Here comes our main modification: spatial connectivity!

# +
prob_distribution = nest.spatial_distributions.gaussian(
    nest.spatial.distance, mean=0., std=0.05)
...
nest.Connect(layer_e, spikes_e)
nest.Connect(layer_i, spikes_i)
...
nest.raster_plot.from_device(spikes_e, hist=False, title='spiking activity of excitatory neurons')
plt.xlim(0, 1000)
plt.ylim(0, NE)

nest.raster_plot.from_device(spikes_i, hist=False, title='spiking activity of inhibitory neurons')
plt.xlim(0, 1000)
plt.ylim(NE, NE+NI)
# -
...
def firing_rate_over_time(neurons, spikes, senders, bin_width = 1000, delta_t = 50, interval=10,
    ...
    plt.colorbar(label=r'rate ($\frac{\mathrm{spikes}}{s}$)')
    plt.xlabel('cortical space x (mm)')
    plt.ylabel('cortical space y (mm)')
